...
function loadSquad() {
    ...
    teamPlayers.forEach(p => {
        const { display } = cleanPlayerName(p.Player);
        ...
        if (p.foto_topscore_x && p.foto_topscore_x.trim() !== "") {
            ...
            // Tenta usar o nome do jogador como arquivo (normalizado)
            const fileName = display.toLowerCase().replace(/[^a-z0-9]/g, "") + ".webp";
        ...
        div.innerHTML = `
            <a href="jogador.html?player=${encodeURIComponent(p.Player)}">
                <div class="squad-player-img">
                    <img src="${imgPath}" alt="${display}" onerror="this.src='assets/images/perfil_jogador/foto_topscore_x/placeholder.webp'">
                </div>
