...
function renderTeammates(allPlayers, currentTeam, currentPlayerName) {
  ...
  track.innerHTML = teammates.map(tm => {
    ...
    return `
      <div class="mu-slide">
        <a href="jogador.html?player=${encodeURIComponent(tm.Player)}" class="tm-card-premium">
...
function initTeammatesSlider() {
  ...
  if (btnNext) btnNext.onclick = () => track.scrollLeft += scrollAmount;
  if (btnPrev) btnPrev.onclick = () => track.scrollLeft -= scrollAmount;
}
